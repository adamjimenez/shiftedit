...
$.extend($.ui.tabs.prototype, {
	overflowResize: function(options) {
		var self = this, initialized = false, buttonWidth, containerWidth, resizeTimer = null, hover = false, dirty = false;
		...
		function doResize(animate) {
			...
			self.tabs.each(function(i) {
				...
				if (tab.hasClass('button')) {
					totalButtonWidth += tab[0].getBoundingClientRect().width;
				}
			});
			
			// hack for button margin
			if (totalButtonWidth) {
				totalButtonWidth += 4;
			}
			...
			var tabMaxWidth = (1/item.length)*100;
			var borderWidth = parseInt($(item[0]).css("border-left-width"), 10) + parseInt($(item[0]).css("border-right-width"), 10);
			var excess = (borderWidth + Math.ceil(totalButtonWidth / self.tabs.length)+1);
			var css = {'max-width' : 'calc('+tabMaxWidth + '% - ' + excess + 'px)'};

			if(animate) {
				var nextWidth = Math.floor(containerWidth/item.length)-excess;
				
				// set widths to fixed as jquery can't aniimate with calc
				item.each(function() {
					$(this).css({
						maxWidth: $(this).width()
					});
				...
				item.animate({
					maxWidth: nextWidth
				}, {duration: 'fast', complete:function(){ item.css(css); } });
			} else {
				item.css(css);
			}
