	...
	$.jstree.plugins.table = function(options,parent) {
		...
		this._initialize = function () {
			if (!this._initialized) {
				var s = this.settings.table || {}, styles,	container = this.element, tableparent = container.parent(), i,
				...
				if (!styled) {
					styled = true;
					styles = [
						'.jstree-table-cell {vertical-align: top; overflow:hidden;margin-left:0;width: 100%;white-space: nowrap; cursor: default;}',
		...
		this.bind = function () {
			...
			.on("ready.jstree",$.proxy(function (e,data) {
				...
				function resize() {
					...
					$('#jsTreeTableHeight').remove();
					$('<style type="text/css" id="jsTreeTableHeight">div.jstree-table-cell-root-'+me.rootid+' {line-height: '+anchorHeight+'px; min-height: '+anchorHeight+'px;}</style>').appendTo("head");
				};
