...
$.extend($.ui.tabs.prototype, {
    _processTabs: function() {
        ...
        if (self.options.closable === true) {
			...
			.appendTo(lis.has('.closable')).hide();
        }
    }
});
...
$(document).on('mouseover', '.closable', function(){
	$(this).find('.tabCloseButton').show().css('opacity', 0.5).children('i').removeClass('fa-circle').addClass('fa-times');
});

$(document).on('mouseout', '.closable', function(){
	if ($(this).data('edited')) {
		$(this).find('.tabCloseButton').show().css('opacity', 1).children('i').removeClass('fa-times').addClass('fa-circle');
	} else {
		$(this).find('.tabCloseButton').hide();
	}
});