...
(function (factory) {
	...
	$.jstree.plugins.grid = function(options,parent) {
		...
		this.bind = function () {
			...
			.on("set_id.jstree",$.proxy(function (e,data) {
				if (data.old !== undefined) {
					var grid = this.gridWrapper, removeNodes = [data.old], i;
					// add children to remove list
					if (data.node && data.node.children_d) {
						removeNodes = removeNodes.concat(data.node.children_d);
					}
					...
					var target = this.get_node(data.node || "#",true);
					this._prepare_grid(target);

					var id = data.node.id;
					findDataCell(this.gridWrapper,id).addClass("jstree-clicked");
					this.get_node(data.node.id,true).children("div.jstree-grid-cell").addClass("jstree-clicked");
				}
			}, this))
		...
		this._prepare_headers = function() {
			...
			if (!this.bound && resizable) {
				...
				$(document).mouseup(function () {
					var  i, ref, cols, headers, currentTree;
					if (isClickedSep) {
						...
						toResize = null;
						for (i=0;i<cols.length;i++) {
							ref._gridSettings.columns[i].width = parseFloat(headers[i].style.width);
						}

						currentTree.trigger("resize_column.jstree-grid", [colNum, parseFloat(headers[colNum].style.width)]);
					}
				...
				this.gridWrapper.on("selectstart", ".jstree-grid-resizable-separator", function () {
					...
					isClickedSep = true;
					oldMouseX = e.pageX;
          colNum = $(this).closest(".jstree-grid-column-root-"+rootid).prevAll(".jstree-grid-column-root-"+rootid).length;
