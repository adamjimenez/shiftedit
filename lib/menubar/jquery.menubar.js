...
(function ($) {
    ...
    $.widget("ui.menubar", {
        ...
        _create: function () {
            ...
            this.items = this.menuItems.children("button, a, div");
        ...
        _initializeWidget: function () {
            ...
            this._on({
                ...
                focusout: function () {
                    this.closeTimer = this._delay(function () {
                        if (this) {
                            if ($.isFunction(this._close) && typeof event !== 'undefined') {
                                this._close(event);
                            }
                            if (this.items && $.isFunction(this.items.attr)) {
                                this.items.attr('tabIndex', -1);
                            }
                            if (this.lastFocused && $.isFunction(this.lastFocused.attr)) {
            ...
            //trigger close when body clicked (Adam Jimenez)
            var me = this;
            $(document).on('click', function(e){
                if ($(e.target).closest('[role=menubar]').length===0)
                    me._close();
            });
        },
        ...
        _initializeMenuItems: function () {
            ...
            subMenus = this.menuItems.children(menubar.options.menus).menu({
                ...
                focus: function (event, ui) {
                	if (ui.item.find('ul').length) {
                		ui.item.parent().css('overflow', 'visible').css('max-height', 'none');
                	} else {
                		ui.item.parent().css('overflow', '').css('max-height', '');
                	}
                },
        ...
        _close: function () {
            ...
            if ($(this.active).prop("tagName") === 'UL') {
